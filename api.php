...
function process_get($param) {
    if($param[0] == "pinjam") {
        ...
        try {
            ...
            if(!empty($param[1])) {
                $handle = $conn->prepare("
                    SELECT member_name, title, loan_date, return_date, due_date FROM member LEFT JOIN loan ON member.member_id = loan.member_id
                ...
                $handle = $conn->query("select * from member");
            }
